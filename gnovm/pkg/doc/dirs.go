...
func (d *bfsDirs) findPackage(name string) []bfsDir {
	...
	// Annotate regular candidates with their testing counterpart.
	...
	// Remaining test dirs have no regular counterpart → testing-only.
...
func (d *bfsDirs) findDir(absdir string) []bfsDir {
	...
	for dir, ok := d.Next(); ok; dir, ok = d.Next() {
		if dir.dir == absdir {
			if d.isUnderTestRoot(dir.dir) {
				dir.testingOnly = true
			} else {
				// Check filesystem for a testing stdlib counterpart.
				for _, root := range d.testRoots {
					td := filepath.Join(root, dir.importPath)
